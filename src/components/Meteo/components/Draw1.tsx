...
export default class Draw implements isAllDownloaded {
            ...
            const graphArray = (selectGroup: number) => {
                ...
                this.dataReduced.forEach((pureData) => {
                    for (
                        ...
                        const source =
                            this.graphsConfig[graphNumber].sourceField

                        if (
                            this.graphsConfig[graphNumber].group === selectGroup
                        ) {
    ...
    get button() {
        ...
        const isXinButton = (x: number) => {
            return {
                startPrev:
                    x >= btnX.startPrev && x <= btnX.startPrev + btnWidth,
                startNext:
                    x >= btnX.startNext && x <= btnX.startNext + btnWidth,
        ...
        const click = (event: MouseEvent | TouchEvent) => {
            ...
            if (y >= 0 && y <= btnHeight) {
                const { startPrev, startNext, endPrev, endNext } =
                    isXinButton(x)
                // decrease month
                if (startPrev) {
    ...
    dataReducer(startOrEnd: string, move: number) {
        ...
        const updatedDate = new Date(
            dateBeforeModification.setDate(
                dateBeforeModification.getDate() + move
            )
        )
    ...
    getInfo(xPos: number, yPos: number) {
        ...
        if (
            ...
            const shortDate = this.getTextDateFromNewDate(
                new Date(dayNumberInMs)
            )
            const dayText = () => {
                // different x text for day and year graph
                if (
                    (this.dataReduced[0][this.dateField] as string).length ===
                    10
                ) {
        ...
        for (
            ...
            if (this.style === 'area') {
                ...
                if (widthOfOneValue > 3) {
                    ...
                    this.ctx.lineWidth =
                        widthOfOneValue < 1 ? 1 : widthOfOneValue
                }
            } else {
            ...
            const line = (oneEntry: pureData) => {
                ...
                if (this.style === 'dot') {
                    //this.ctx.beginPath();
                    this.ctx.moveTo(
                        this.xPositionFromDate(
                            oneEntry[this.dateField] as string
                        ) + this.lineWidth,
                        -this.lineWidth / 2 +
